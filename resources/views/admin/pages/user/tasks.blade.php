                                ...
                                <tr>
                                    ...
                                    <td width="20%">
                                        @if($entity->hasSubscribed($_entity->id))
                                            <a class="btn btn-danger" href="{{ route('admin.users.tasks.unsubscribe',['user' => $_entity, 'task' => $entity]) }}">{{ ___('Отписать') }}</a>
                                            @else
                                            <a class="btn btn-success" href="{{ route('admin.users.tasks.subscribe',['user' => $_entity, 'task' => $entity]) }}">{{ ___('Подписать') }}</a>
                                        @endif
                                            @if($entity->certificate)
                                            @if($entity->hasCertificate($_entity->id))
                                                <a class="btn btn-danger" href="{{ route('admin.users.tasks.delete_certificate_task',['user' => $_entity, 'task' => $entity]) }}">{{ ___('Отозвать сертификат') }}</a>
                                            @else
                                                <a class="btn btn-success" href="{{ route('admin.users.tasks.generate_certificate_task',['user' => $_entity, 'task' => $entity]) }}">{{ ___('Сгенерировать сертификат') }}</a>
                                            @endif
                                            @else
