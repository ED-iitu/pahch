...
dragula([document.getElementById("left-events"), document.getElementById("right-events")])
  ...
  .on('drop', function (el, target, source, sibling) {
    ...
    element.className = "media d-block d-sm-flex empty-media";
    element.style = "border: none;min-height: 90px;max-height: 90px;";
    if(source.id === "left-events"){
      document.querySelector("#left-events").append(element)
      if(document.querySelector("#right-events .empty-media")){
        document.querySelector("#right-events .empty-media").remove();
      }
    }
    if(source.id === "right-events"){
      document.querySelector("#right-events").append(element)
      if(document.querySelector("#left-events .empty-media")){
        document.querySelector("#left-events .empty-media").remove();
      }
    }
  })
